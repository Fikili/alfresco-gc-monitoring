...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f gc-monitor-share
	call %MVN_EXEC% clean package -pl gc-monitor-share,gc-monitor-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill gc-monitor-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f gc-monitor-acs
	call %MVN_EXEC% clean package -pl gc-monitor-integration-tests,gc-monitor-platform,gc-monitor-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl gc-monitor-platform,gc-monitor-integration-tests,gc-monitor-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl gc-monitor-platform,gc-monitor-integration-tests
EXIT /B 0
:purge
